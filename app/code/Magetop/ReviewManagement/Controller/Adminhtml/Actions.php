...
abstract class Actions extends \Magento\Backend\App\Action
{
    ...
    {
        $data = \Magento\Framework\App\ObjectManager::getInstance()->create('Magento\Eav\Model\Entity\Attribute')->getCollection();
        $attribute = array();
        foreach($data->getData() as $dt){
            $attribute[$dt['attribute_id']] = $dt['attribute_code'];
        } 
        $ids = $this->getRequest()->getParam($this->_idKey);
