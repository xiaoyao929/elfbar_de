...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getWishListProductIds() {
        ...
        $isLoggedIn = $context->getValue(Context::CONTEXT_AUTH);
        if(!$isLoggedIn){
            return $pids;
        }
...
$resource = $objectManager->get('Magento\Framework\App\ResourceConnection');
        ...
        $sql = "Select * FROM wishlist w join wishlist_item wi on w.wishlist_id = wi.wishlist_id WHERE w.customer_id = " . $customerId;
        $result = $connection->fetchAll($sql);
        $pids = array_column($result, "product_id");       
return $pids;
